...
public class EmployeeHomeController implements Initializable {
	...
	private void addsDate(ActionEvent event) {
		...
		String date = datePicker.getValue().format(DateTimeFormatter.ofPattern("dd-MM-YY"));    //GETS THE DATE SELECTED
		...
		String dateTime = date + " " + time;										//CONCATENATES DATE AND TIME SELECTED

		int x = 0;
		if (today.compareTo(selectedDate) >= 0)										//CHECKS IF DATE SELECTED IS IN THE PAST
			x = 1;

		StringBuffer sb = new StringBuffer("");									//STRING BUFFER TO COLLECT THE DATE/TIME INFORMATION 
																				//FOR FILMS ALREADY BEING SHOWN
		int a = 0;
		
		if (list != null) {

			for (int i = 0; i < list.size(); i++) {								//ITERATES THROUGH THE LIST OF XML NODES IN 'film.xml'
				Element node = (Element) list.get(i);							//THAT WAS PARSED ON INITIALIZATION
				String[] dT = node.getChildText("dateTimes").split(",");		//GETS THE DATE/TIME INFORMATOIN FOR EACH FILM
				...
				for (String element : dT) {	
					...
					sb.append(element + "\n");									//ADDS THE DATE/TIME INFORMATION IN 'film.xml' TO THE STRING BUFFER
				}
			}
		}
	...
	/**
	 * Opens a FileChooser for employee to pick a film image. Writes the relative path of the image
	 * to a lavel to write to XML.
	 ...
	 * to set the film variables. Calls <code>cinema.XML.CreateFilmXML.createFilm()</code>.
	...
	private void addFilm(ActionEvent event) {
		...
		default:
			...
			String[] arrDateTimes = dateTimes.toString().split(",");    //GETS A LIST OF ALL DATES/TIMES A FILM IS BEING SHOWN
			for (int i = 0; i < arrDateTimes.length; i++) {				//ITERATES THROUGH THIS LIST TO CREATE A BOOKING XML NODE FOR EACH SHOWING OF THE NEW FILM
			...
			Alert alert2 = new Alert(AlertType.CONFIRMATION);			//CONFIRMATION ALERT WHEN FILM HAS BEEN ADDED SUCCESSFULLY
